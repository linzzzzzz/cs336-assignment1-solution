...
def test_encode_iterable_memory_usage():
    ...
    with open(FIXTURES_PATH / "tinystories_sample_5M.txt") as f:
        ...
        for _id in _encode_iterable(tokenizer, f):
            ids.append(_id)
            count += 1
            # Print progress every 10,000 tokens
            if count % 10000 == 0:
                elapsed_time = time.time() - start_time
                tokens_per_second = count / elapsed_time
                avg_time_per_token = elapsed_time / count * 1000  # in milliseconds
                print(f"Progress: {count} tokens processed, {tokens_per_second:.1f} tokens/sec, {avg_time_per_token:.3f} ms/token")
        
        total_time = time.time() - start_time
        final_tokens_per_second = len(ids) / total_time
        final_avg_time_per_token = total_time / len(ids) * 1000
        print(f'Final: {len(ids)} tokens, {final_tokens_per_second:.1f} tokens/sec, {final_avg_time_per_token:.3f} ms/token')
